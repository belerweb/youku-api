...
public abstract class YoukuAPI {
	...
	protected Configuration config = YoukuAPIConfig.getInConfiguration();
	 ...
	 * @return 搜索结果
	 */
	public abstract Object searchVideoByKeyword(String[] keywords, Integer page, Integer pageSize, Integer orderBy,
			Integer[] categorieIds, Integer searchScope, String fileType, Integer relatedWordsNum,
			Boolean advanceSearch, Integer limitDate);

	public abstract Object searchVideoByTag(String[] tags, Integer page, Integer pageSize, Integer orderBy,
			Integer[] categorieIds, Integer searchScope);

	protected String buikdSearchVideoByKeyword(String[] keywords, int returnType, Integer page, Integer pageSize,
			Integer orderBy, Integer[] categorieIds, Integer searchScope, String fileType, Integer relatedWordsNum,
			Boolean advanceSearch, Integer limitDate) {
		if (keywords == null || keywords.length == 0) {
			throw new RuntimeException("searchVideoByKeyword 至少需要一个关键词。");
		}
		ArrayList<String> tmp = new ArrayList<String>();
		tmp.add(ParameterConstant.PID + "=" + config.getString(ParameterConstant.PID));
		try {
			tmp.add(ParameterConstant.SEARCH_VALUE + "=" + URLEncoder.encode(StringUtils.join(keywords, " "), "UTF-8"));
		...
		if (orderBy != null && (orderBy == 1 || orderBy == 2 || orderBy == 3 || orderBy == 4 || orderBy == 6)) {
	...
	protected String buikdSearchVideoByTag(String[] tags, int returnType, Integer page, Integer pageSize,
		...
		tmp.add(ParameterConstant.PID + "=" + config.getString(ParameterConstant.PID));
		try {
			tmp.add(ParameterConstant.SEARCH_VALUE + "=" + URLEncoder.encode(StringUtils.join(tags, " "), "UTF-8"));
		} catch (UnsupportedEncodingException e) {
			// Ignore exception
		}
		tmp.add(ParameterConstant.RETURN_TYPE + "=" + returnType);
